...
final public class StreamAddressSpace {
    ...
    public StreamAddressSpace getAddressesInRange2(StreamAddressRange range) {
        ...
        StreamAddressSpace ret = new StreamAddressSpace(this.trimMark, addressesInRange);
        StreamAddressSpace other = getAddressesInRange(range);

        if (ret.getTrimMark() != other.getTrimMark() && !Arrays.equals(ret.toArray(), other.toArray())) {
    ...
    public StreamAddressSpace getAddressesInRange(StreamAddressRange range) {
        ...
        if (this.bitmap.isEmpty() || range.getEnd() >= last || range.getStart() < first) {
            // Return empty address space if the range doesn't intersect, or if the bitmap is empty
            return  new StreamAddressSpace(this.trimMark, addressesInRange);
        }

        long leftEndpoint = Math.max(first, range.getEnd() + 1);
        long rightEndpoint = Math.min(last, range.getStart());
